...
BEGIN
    ...
    IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'team_members') THEN
        ...
        IF NOT EXISTS (SELECT 1 FROM information_schema.columns 
                       WHERE table_name = 'company_team_members' AND column_name = 'company_role') THEN
            ALTER TABLE company_team_members ADD COLUMN company_role VARCHAR(50);
            -- نسخ البيانات من role إلى company_role
            UPDATE company_team_members SET company_role = role;
        END IF;
        
        IF NOT EXISTS (SELECT 1 FROM information_schema.columns 
                       WHERE table_name = 'company_team_members' AND column_name = 'company_status') THEN
            ALTER TABLE company_team_members ADD COLUMN company_status VARCHAR(20);
            -- نسخ البيانات من status إلى company_status
            UPDATE company_team_members SET company_status = status;
        END IF;
    ...
    ELSE
        ...
        CREATE TABLE company_team_members (
            ...
            phone VARCHAR(50),
            company_role VARCHAR(50) DEFAULT 'member' CHECK (company_role IN ('manager', 'lead', 'editor', 'designer', 'member')),
            company_status VARCHAR(20) DEFAULT 'active' CHECK (company_status IN ('active', 'inactive')),
...
-- 11. إنشاء view لسهولة الاستعلام
...
SELECT 
    ...
    ctm.phone,
    ctm.avatar_url,
    ctm.company_role, -- دور العضو في الشركة
    ctm.company_status, -- حالة العضو في الشركة
    
    -- بيانات المشروع
    p.name as project_name,
    p.status as project_status_overall,
...
-- 12. إنشاء دالة مساعدة للحصول على أعضاء المشروع
...
BEGIN
    ...
    SELECT 
        ...
        ctm.company_role as company_role,
        ctm.company_status as company_status,
        ptm.project_role,
        ptm.project_status,
        ptm.joined_project_date,
        ptm.project_notes,
