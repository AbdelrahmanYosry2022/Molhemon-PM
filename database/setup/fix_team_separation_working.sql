...
BEGIN
    ...
    ELSIF NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'company_team_members') THEN
    ...
    ELSE
        RAISE NOTICE 'جدول company_team_members موجود بالفعل، سيتم المتابعة';
    END IF;
END $$;
...
-- 11. إنشاء view لسهولة الاستعلام (مصحح نهائياً)
...
SELECT 
    ...
    ctm.phone,
    ctm.avatar_url,
    ctm.role as company_role, -- دور العضو في الشركة
    ctm.status as company_status, -- حالة العضو في الشركة
