-- إضافة الأعمدة المفقودة لجدول team_members
...
-- إنشاء دالة لتحديث updated_at تلقائياً (إذا لم تكن موجودة)
...
INSERT INTO team_members (name, first_name, last_name, email, phone, role, status, joined, bio, created_at) 
SELECT 
    'أحمد محمد', 'أحمد', 'محمد', 'ahmed@example.com', '+201234567890', 'manager', 'active', '2024-01-15', 'مدير مشاريع ذو خبرة 10 سنوات', NOW()
WHERE NOT EXISTS (SELECT 1 FROM team_members WHERE email = 'ahmed@example.com');

INSERT INTO team_members (name, first_name, last_name, email, phone, role, status, joined, bio, created_at) 
SELECT 
    'فاطمة علي', 'فاطمة', 'علي', 'fatima@example.com', '+201234567891', 'lead', 'active', '2024-02-01', 'قائدة فريق تطوير', NOW()
WHERE NOT EXISTS (SELECT 1 FROM team_members WHERE email = 'fatima@example.com');

INSERT INTO team_members (name, first_name, last_name, email, phone, role, status, joined, bio, created_at) 
SELECT 
    'محمد حسن', 'محمد', 'حسن', 'mohamed@example.com', '+201234567892', 'designer', 'active', '2024-02-15', 'مصمم جرافيك محترف', NOW()
WHERE NOT EXISTS (SELECT 1 FROM team_members WHERE email = 'mohamed@example.com');

INSERT INTO team_members (name, first_name, last_name, email, phone, role, status, joined, bio, created_at) 
SELECT 
    'سارة أحمد', 'سارة', 'أحمد', 'sara@example.com', '+201234567893', 'editor', 'active', '2024-03-01', 'محررة محتوى', NOW()
WHERE NOT EXISTS (SELECT 1 FROM team_members WHERE email = 'sara@example.com');

INSERT INTO team_members (name, first_name, last_name, email, phone, role, status, joined, bio, created_at) 
SELECT 
    'علي محمود', 'علي', 'محمود', 'ali@example.com', '+201234567894', 'member', 'active', '2024-03-15', 'مطور ويب', NOW()
WHERE NOT EXISTS (SELECT 1 FROM team_members WHERE email = 'ali@example.com');

INSERT INTO team_members (name, first_name, last_name, email, phone, role, status, joined, bio, created_at) 
SELECT 
    'نور الدين', 'نور', 'الدين', 'nour@example.com', '+201234567895', 'member', 'inactive', '2024-01-01', 'مطور تطبيقات', NOW()
