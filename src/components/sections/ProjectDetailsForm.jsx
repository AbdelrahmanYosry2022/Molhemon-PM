...
// --- استقبال قائمة العملاء والعميل المحدد ---
function ProjectDetailsForm({ project, updateProject, currency, clients }) {
  return (
    <div className={`lg:col-span-2 bg-[${colors.background}] p-4 rounded-xl border border-[${colors.border}] shadow-sm`}>
      ...
      <div className="grid sm:grid-cols-2 gap-4">
        ...
        {/* --- القائمة المنسدلة الجديدة لاختيار العميل --- */}
        <label className="space-y-1">
          <span className={`text-sm font-medium text-[${colors.textSecondary}]`}>العميل</span>
          <select 
            className={`w-full rounded-md border border-[${colors.border}] bg-white px-3 py-2 focus:border-[${colors.primary}] focus:ring-2 focus:ring-[${colors.primaryMuted}] outline-none`} 
          ...
          >
            ...
            {clients.map((c) => (
              <option key={c.id} value={c.id}>
                {c.prefix ? `${c.prefix} ` : ''}{c.first_name} {c.last_name} {c.industry ? `(${c.industry})` : ''}
              </option>
            ))}
          </select>
        </label>
